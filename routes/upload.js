...
router.post('/process', upload.single('image-crop'), (req, res) => {
	...
	sleep(7000, function() {
		exec('copy predictions.jpg public\\results\\images\\crop-'+dateToString(now)+'.jpg', (error, stdout, stderr) => {
    })
    
    let data = {
      file_original   : filename,
      file_prediction : 'crop-'+dateToString(now)+'.jpg',
      created_at      : new Date()
    }
    ...
    db.collection("data").insertOne(data, (err, result) => {
      ...
      res.render('image/result', {
        file_original: filename,
        file_box: 'crop-'+dateToString(now)+'.jpg',
      })
    })
	})
});
